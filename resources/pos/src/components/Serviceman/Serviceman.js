...
const Serviceman = (props) => {
    ...
    const { fetchServiceman, fetchServiceManFromIndexDB, serviceman, selectedLocationId } = props;
    ...
    const OnClickSelectColor = (full_name) => {
        setSelectChosenColor(full_name);
    };
                                                ...
                                                {serviceman.service_staff && serviceman.service_staff.map((serviceMan, index) => {
                                                    return (
                                                        <button type="button" key={index}
                                                            className={`${SelectChosenColor === serviceMan.full_name ? "service-active" : ""} serviceman-modal__service-btn btn btn-outline-dark d-flex align-items-center justify-content-center`}
                                                            onClick={() => OnClickSelectColor(serviceMan.full_name)}
                                                        >
