...
const CloseRegisterDetail = (props) => {
    ...
    useEffect(() => {
        if (closeRegisters.length !== 0) {
            setTotalPayment(closeRegisters.register_details.total_cash ? (parseFloat(closeRegisters.register_details.total_cash) + parseFloat(closeRegisters.register_details.cash_in_hand) - parseFloat(closeRegisters.register_details.total_cash_refund)).toFixed(2) : 0.00);
        }
    }, [closeRegisters])
