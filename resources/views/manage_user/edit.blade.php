            ...
            <div class="col-md-9">
                <div class="col-md-4">
                    <div class="checkbox">
                        ...
                        [ 'class' => 'input-icheck','id' => 'all']); !!} {{ __( 'role.all_locations' ) }} 
                        </label>
                ...
                if (is_array($permitted_locations)) {
                    $PermissionCount = count($permitted_locations);
                }
                ?>
                @if(in_array('kitchen',$business->enabled_modules))
                <div class="col-sm-4" style="margin-bottom: -35px;">
                    <div class="form-group">
                        {!! Form::label('kitchen_station',  __( 'messages.kitchen_stations' ).' :') !!}
                        {!! Form::select('kitchen_station_id', $kitchenStations, $user->kitchen_station_id, ['class' => 'form-control select2','id' => 'kitchenStation','placeholder' => __('lang_v1.none'), $permitted_locations == 'all' || empty($permitted_locations) || $PermissionCount > 1 ? 'disabled' : '', 'data-id' => ($permitted_locations != 'all') ? count($permitted_locations) > 1 ? '' : last($permitted_locations) : '']); !!}
                    </div>
                </div>
                @endif
...
<script type="text/javascript">
  ...
  locations = [];
  let id = $('#kitchenStation').data('id');
  let station = $('#kitchenStation').val();
  if (id != ''){
      locations.push(id);
      get_kitchen_station(id);
  }

  $('input').on('ifChecked', function(event){
      locations = locations.filter(function(v){return v!==''});
      let location = $(this).val();
      if(location.indexOf('location.') !== -1){
          locations.push(location.substr(-1));
          get_kitchen_station(location.substr(-1));
      }

      if($('#all').prop('checked')){
          $('#kitchenStation').attr('disabled',true);
      }else if(locations.length > 1){
          $('#kitchenStation').attr('disabled',true);
      }else{
          $('#kitchenStation').attr('disabled',false);
      }
  });
  ...
  $('input').on('ifUnchecked', function(event){
      ...
      if(location.indexOf('location.') !== -1){
          locations = jQuery.grep(locations, function(value) {
              return value != location.substr(-1);
          });
          get_kitchen_station(locations);
      }
  ...
  function get_kitchen_station(locationId){
      $.ajax({
          url: '/kitchen-stations',
          type: 'GET',
          data :{location_id : [locationId]},
          success: function (result) {
              $('#kitchenStation').empty();
              const stations = result.data;
              $('#kitchenStation').append('<option value="">None</option>');
              for (const key in stations) {
                  if (stations.hasOwnProperty(key)) {
                      $('#kitchenStation').append($('<option>', {value: key, text: stations[key]}));
                  }
              }
              $('#kitchenStation').val(station).trigger('change');
              station = '';
          }
      });
  }
