    ...
    @component('components.filters', ['title' => __('report.filters')])
        ...
        <div class="col-md-3">
            <div class="form-group">
                ...
                {!! Form::select('category_id', $categories, $categoryID_filter, ['class' => 'form-control select2', 'style' => 'width:100%', 'id' => 'product_list_filter_category_id', 'placeholder' => __('lang_v1.all')]); !!}
            </div>
        </div>
...
@section('javascript')
    ...
    <script type="text/javascript">
                                ...
                                $.ajax({
                                    ...
                                    success: function(result) {
                                        if (result.success == true) {
                                            ...
                                            .val('');
                                            $('.select-all-product-row').prop('checked', false).change();
                                        } else {
                                            toastr.error(result.msg);
                                        }
                                    },
        ...
        $(document).on('change', 'input.select-row-product', function() {
            let selectRowProduct = $('#product_table  input:checkbox:checked').length;
            let tbodyTotalCheckbox = $('#product_table tbody input:checkbox').length;
            if($('.select-all-product-row:checkbox:checked')){
                if (selectRowProduct < tbodyTotalCheckbox){
                    $('.select-all-product-row').prop('checked', false).change();
                }
            }
            if (this.checked) {
        ...
        // select all row
        $(document).on('click', '.select-all-product-row', function(e) {
            if (this.checked) {
                $(this)
                    .closest('table').parent().parent().next().find('table').find('tbody')
