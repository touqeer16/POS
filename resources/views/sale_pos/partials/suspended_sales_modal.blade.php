...
<div class="modal-dialog modal-lg" role="document">
	<div class="modal-content">
		...
		<div class="modal-body">
            @if(!empty($salesIds))
            <div class="row">
                <button value="{{ !empty(json_encode($salesIds)) ? json_encode($salesIds) : ''  }}" class="small-box-footer delete-all-sale-button bg-red is_suspended  padding-10" style="float: right;border: none;margin-right: 10px;">
                    @lang('messages.delete') @lang('messages.all') <i class="fas fa-trash"></i>
                </button>
            </div>
            @endif
			<div class="row">
				...
				@forelse($sales as $sale)
					@if($sale->is_suspend)
						<div class="col-xs-6 col-sm-3 suspendedSalesDiv">
				...
				@endforelse
                <p class="text-center noRecordFound"></p>
			</div>
		</div>
