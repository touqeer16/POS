...
<section class="content">
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="col-sm-12" style="margin-top: 0px;padding-left: 0px;" id="StationDiv">
                                ...
                                @else
                                    ...
                                    @endforeach
                                @endif
                                  <input type="hidden" name="deleted_station_name" class="form-control deleted_station_name" placeholder="deletedd Station Name"  >
                            </div>
                            ...
                            <div class="row">
                                <table class="table table-bordered table-striped  " role="grid">
                                    <thead>
                                        <tr>
                                            <th> @lang('lang_v1.station_name')</th>
                                             <th> @lang('lang_v1.assign_unassign_categories') </th>
                                            <th>  @lang('lang_v1.assigned_category_name')</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @if($collectionKitchens)
                                            @foreach($collectionKitchens as $key => $kitchens)
                                            <tr>
                                                @foreach($kitchens as $key => $kitchenStation)
                                                    <td>{{$kitchenStation->station_name}}</td>
                                                    <td>
                                                        <button data-href="/business-location/{{$kitchenStation->id}}/{{$location->id}}/assign_kitchen_categories" type="button" class="btn btn-outline-success mx-auto btn-lg assign_unassign_categories_button " > @lang('lang_v1.assign_unassign_categories') <i class="fas fa-plus"></i></button>
                                                    </td>
                                                    <td>
                                                        @foreach($kitchenStation->kitchenCategory as $key => $kitchenCategory)

                                                            <span class="badge badge-pill badge-info">{{$kitchenCategory->name}}</span>
    ...
    <div class="modal fade addKitchenCategoryModal" tabindex="-1" role="dialog" aria-labelledby="gridSystemModalLabel"></div>
...
@section('javascript')
    <script type="text/javascript">
        ...
        $(document).on('click','.btn-delete', function(){
            ...
            let deleted_station_name = $('.deleted_station_name').val();
            $('.deleted_station_name').val(id);

            if((deleted_station_name)){
                $('.deleted_station_name').val(id + ',' + deleted_station_name);
            } else{
                $('.deleted_station_name').val(id );
            }
        ...
        $(document).on('click', 'button.assign_unassign_categories_button', function() {
            $('div.addKitchenCategoryModal').load($(this).data('href'), function() {
                $(this).modal('show');
                $('form#assign_unassign_kitchen_form').submit(function(e) {
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            if (result.success === true) {
                                ...
                                $('div.addKitchenCategoryModal').modal('hide');
