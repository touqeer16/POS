...
if (! function_exists('getReceiptUrl')) {
    /**
     * @param $receiptData
     *
     * @return string
     */
    function getReceiptUrl($receiptData)
    {
        $path =  storage_path('/../../public_html/uploads/receipts/');
        if (! File::exists($path)) {
            File::makeDirectory($path);
        }
        $randomNumber = Str::random(10);
        while (true) {
            $generateRandomFile = 'receipt_'.$randomNumber.'.html';
            $newReceiptFile = $path.$generateRandomFile;
            $receiptFileExists = file_exists($newReceiptFile);
            if ($receiptFileExists) {
                getReceiptUrl($receiptData);
            } else {
                $file = fopen($newReceiptFile, "w");
                rewind($file);
                fwrite($file, $receiptData);
                fflush($file);
                fclose($file);
            }
            break;
...
if (! function_exists('countTotalTax')) {
     ...
     */
    function countTotalTax($receiptData)
    {
        $totalAmount = 0;
        foreach ($receiptData->appliedTax as $data){
            $totalAmount = $totalAmount + ceil($data['tax_amount'] * 100) / 100;
        }
...
if (! function_exists('getModifierName')) {
     ...
     * @return string
     */
    function getModifierName($modifiers)
    {
        $modifierName = [];
        foreach ($modifiers as $modifier){
            if($modifier['unit_price_inc_tax'] == 0 || $modifier['unit_price_inc_tax'] <= 0){
                array_push($modifierName, $modifier['variation']);
            }
        }
        ...
        if ($modifierName > 0){
            return '
                <tr>
                    <td> </td>
                    <td style="font-size: 13px;">'.  join(", ",$modifierName)  .'</td>
                    <td style="text-align: end; padding: 0; font-size: 14px;"></td>
                    <td style="text-align: end; padding: 0; font-size: 14px;"></td>
                    <td style="text-align: end; padding: 0; font-size: 14px;"></td>
                </tr>
            ';
