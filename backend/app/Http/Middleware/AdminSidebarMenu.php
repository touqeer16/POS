...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            $common_settings = !empty(session()->get('business.common_settings')) ? session()->get('business.common_settings') : [];
            ...
            if (auth()->user()->can('user.view') || auth()->user()->can('user.create') || auth()->user()->can('roles.view')) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabledMenuLink) {
                        ...
                        /* ! */
                        if (in_array('sales_commission_agents', $enabledMenuLink) && auth()->user()->can('user.create')) {
                            $sub->url(
            ...
            if (auth()->user()->can('supplier.view') || auth()->user()->can('customer.view')) {
                $menu->dropdown(
                    __('contact.contacts'),
                    function ($sub) use ($enabledMenuLink) {
                        if (!in_array('supplier', $enabledMenuLink) && auth()->user()->can('supplier.view')) {
                            $sub->url(
                        ...
                        if (!in_array('customer_groups', $enabledMenuLink) && auth()->user()->can('customer.view')) {
                            $sub->url(
                        ...
                        if (!in_array('import_contacts', $enabledMenuLink) && (auth()->user()->can('supplier.create') || auth()->user()->can('customer.create'))) {
                            $sub->url(
                        ...
                        if (!in_array('map', $enabledMenuLink) && !empty(env('GOOGLE_MAP_API_KEY'))) {
                            $sub->url(
            ...
            if (auth()->user()->can('product.view') || auth()->user()->can('product.create') ||
                ...
                $menu->dropdown(
                    __('sale.products'),
                    function ($sub) use ($enabled_modules, $common_settings, $enable_brand, $enabledMenuLink) {
                        ...
                        /* ! */
                        if (!in_array('print_labels', $enabledMenuLink) && auth()->user()->can('product.view')) {
                            $sub->url(
                        ...
                        /* ! variation */
                        if (!in_array('variations', $enabledMenuLink) && auth()->user()->can('product.create')) {
                            $sub->url(
                        ...
                        if (!in_array('import_products', $enabledMenuLink) && auth()->user()->can('product.create')) {
                            /* ! */
                            $sub->url(
                        ...
                        /* ! */
                        if (!in_array('import_opening_stock', $enabledMenuLink) && auth()->user()->can('product.opening_stock')) {
                            $sub->url(
                        ...
                        if (in_array('selling_price_group', $enabledMenuLink) && auth()->user()->can('product.create')) {
                            $sub->url(
                        ...
                        /* ! */
                        if ($enable_brand == 1 && (auth()->user()->can('brand.view') || auth()->user()->can('brand.create'))) {
                            $sub->url(
                        ...
                        if (array_key_exists('enable_product_warranty', $common_settings)) {
                            $sub->url(
            ...
            //Purchase dropdown
            /* ! */
            if (!in_array('purchases', $enabledMenuLink) && in_array('purchases', $enabled_modules) && (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create') || auth()->user()->can('purchase.update'))) {
                $menu->dropdown(
            ...
            if (auth()->user()->can('sell.view') || auth()->user()->can('sell.create') || auth()->user()->can('direct_sell.access') || auth()->user()->can('view_own_sell_only') || auth()->user()->can('access_sell_return')) {
                $menu->dropdown(
                    __('sale.sale'),
                    function ($sub) use ($enabled_modules, $enabledMenuLink) {
                        ...
                        if (!in_array('pos_sale', $enabledMenuLink) && auth()->user()->can('sell.create')) {
                            $sub->url(
                        ...
                        if (auth()->user()->can('sell.create')) {
                            $sub->url(
                                '/pos/new',
                                __('sale.new_pos_sale'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'pos' && request()->segment(2) == 'new']
                            );
                        }
                        /* ! */
                        ...
                        if (!in_array('add_quotation', $enabledMenuLink) && in_array('add_sale', $enabled_modules) && auth()->user()->can('direct_sell.access')) {
                            $sub->url(
                        ...
                        if (!in_array('list_quotations', $enabledMenuLink) && auth()->user()->can('list_quotations')) {
                            $sub->url(
                        ...
                        if (!in_array('list_sell_return', $enabledMenuLink) && auth()->user()->can('access_sell_return')) {
                            $sub->url(
                        ...
                        if (!in_array('shipments', $enabledMenuLink) && auth()->user()->can('access_shipping')) {
                            $sub->url(
                        ...
                        if (!in_array('import_sales', $enabledMenuLink) && auth()->user()->can('sell.create')) {
                            $sub->url(
            ...
            if (!in_array('stock_transfers', $enabledMenuLink) && in_array('stock_transfers', $enabled_modules) && (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create'))) {
                $menu->dropdown(
            ...
            if (!in_array('stock_adjustment', $enabledMenuLink) && in_array('stock_adjustment', $enabled_modules) && (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create'))) {
                $menu->dropdown(
            ...
            //Accounts dropdown
            if (!in_array('payment_accounts', $enabledMenuLink) && auth()->user()->can('account.access') && in_array('account', $enabled_modules)) {
                $menu->dropdown(
            ...
            if (auth()->user()->can('purchase_n_sell_report.view') || auth()->user()->can('contacts_report.view')
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $enabledMenuLink) {
                        ...
                        if (!in_array('tax_report', $enabledMenuLink) && auth()->user()->can('tax_report.view')) {
                            $sub->url(
                        ...
                        /* ! */
                        if (!in_array('getCustomerSuppliers', $enabledMenuLink) && auth()->user()->can('contacts_report.view')) {
                            $sub->url(
                        ...
                        if (auth()->user()->can('stock_report.view')) {
                            if (!in_array('stock_report', $enabledMenuLink)) {
                                $sub->url(
                            ...
                            if (!in_array('stock_expiry_report', $enabledMenuLink) && session('business.enable_product_expiry') == 1) {
                                $sub->url(
                            ...
                            /* ! */
                            if (!in_array('lot_report', $enabledMenuLink) && session('business.enable_lot_number') == 1) {
                                $sub->url(
                            ...
                            if (!in_array('stock_adjustment_report', $enabledMenuLink) && in_array('stock_adjustment', $enabled_modules)) {
                                $sub->url(
                        ...
                        if (!in_array('items_report', $enabledMenuLink) && auth()->user()->can('purchase_n_sell_report.view')) {
                            /* ! */
                            $sub->url(
                        ...
                        if (!in_array('product_purchase_report', $enabledMenuLink) && auth()->user()->can('purchase_n_sell_report.view')) {
                        ...
                        /* ! */
                        if (!in_array('purchase_payment_report', $enabledMenuLink) && auth()->user()->can('purchase_n_sell_report.view')) {
                            $sub->url(
                        ...
                        if (!in_array('sales_representative', $enabledMenuLink) && auth()->user()->can('sales_representative.view')) {
                            $sub->url(
                        ...
                        if (!in_array('table_report', $enabledMenuLink) && auth()->user()->can('purchase_n_sell_report.view') && in_array('tables', $enabled_modules)) {
                            $sub->url(
                        ...
                        if (!in_array('service_staff_report', $enabledMenuLink) && auth()->user()->can('sales_representative.view') && in_array('service_staff', $enabled_modules)) {
                            $sub->url(
            ...
            //Backup menu
            if (!in_array('backup', $enabledMenuLink) && auth()->user()->can('backup')) {
                $menu->url(action('BackUpController@index'), __('lang_v1.backup'), ['icon' => 'fa fas fa-hdd', 'active' => request()->segment(1) == 'backup'])->order(60);
            }

            //Modules menu
            if (!in_array('modules', $enabledMenuLink) && auth()->user()->can('manage_modules')) {
            ...
            //Booking menu
            if (!in_array('bookings', $enabledMenuLink) && in_array('booking', $enabled_modules) && (auth()->user()->can('crud_all_bookings') || auth()->user()->can('crud_own_bookings'))) {
            ...
            //Kitchen menu
            if (!in_array('kitchen', $enabledMenuLink) && in_array('kitchen', $enabled_modules)) {
            ...
            //Service Staff menu
            if (!in_array('orders', $enabledMenuLink) && in_array('service_staff', $enabled_modules)) {
            ...
            if (!in_array('notification_templates', $enabledMenuLink) && auth()->user()->can('send_notifications')) {
            ...
            if (auth()->user()->can('business_settings.access') ||
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $enabledMenuLink) {
                        ...
                        /* ! */
                        if (!in_array('barcode_settings', $enabledMenuLink) && auth()->user()->can('barcode_settings.access')) {
                            $sub->url(
                        ...
                        /* ! */
                        if (!in_array('receipt_printers', $enabledMenuLink) && auth()->user()->can('access_printers')) {
                            $sub->url(
                        ...
                        if (!in_array('tables', $enabledMenuLink) && in_array('tables', $enabled_modules) && auth()->user()->can('access_tables')) {
                            $sub->url(
                        ...
                        if (!in_array('types_of_service', $enabledMenuLink) && in_array('types_of_service', $enabled_modules) && auth()->user()->can('access_types_of_service')) {
                            $sub->url(
