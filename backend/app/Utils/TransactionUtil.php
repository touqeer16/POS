...
class TransactionUtil extends Util
{
    ...
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        ...
        if (!empty($edit_ids)) {
            ...
            $combo_delete_lines = TransactionSellLine::whereIn('parent_sell_line_id',
                $deleted_lines)->where('children_type', 'combo')->select('id')->get()->toArray();
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if (!empty($location_id)) {
            ...
            if (!empty($business_id)) {
                $kitchenStations = KitchenStation::where('business_location_id', $location_id)->where('business_id', $business_id)->get();
                if (sizeof($kitchenStations)) {
                    $kitchenExist = true;
                    foreach ($kitchenStations as $Pkey => $kitchen) {
                        ...
                        /* echo "<pre>";
                        print_r($KitchenStationCategory); */
        ...
        $output = [
            ...
            'location_id' => ($il->show_location_name == 1) ? $location_details->id : '',
            'business_id' => ($il->show_location_name == 1) ? $business_details->id : '',
        ...
        //Invoice info
        $output['invoice_no'] = (int) $transaction->invoice_no;
        $output['clientCompany'] = $transaction->clientCompany ?? "";
        $output['clientCompanyLabel'] = "Client Name";
        ...
        if ($transaction_type == 'sell') {
            ...
            // Business
            $output['receipt_printer_type'] = $location_details->receipt_printer_type;
            $output['number_of_thermal_print_copy'] = $location_details->number_of_thermal_print_copy;
            ...
            foreach ($lines as $line) {
                foreach ($line->modifiers as $modifier) {
                    if (!is_null($modifier->variations->tax_id) && $modifier->variations->tax_type == 'inclusive') {
                        $taxRate = TaxRate::find($modifier->variations->tax_id);
                        array_push($modifierTaxes,
                            $line->quantity * ($modifier->variations->default_sell_price * $taxRate->amount / 100));
                    }
                }
                $tax_details = TaxRate::find($line->tax_id);
                if (!empty($tax_details)) {
                    $appliedTaxCollection = collect($appliedTax)->firstWhere('tax_name', $tax_details->name);
                    if ($appliedTaxCollection) {
                        $singleProductTax = $line->unit_price * $tax_details->amount / 100;
                        $appliedTaxKey = array_search($appliedTaxCollection['tax_name'], array_column($appliedTax, 'tax_name'));
                        $appliedTax[$appliedTaxKey]['tax_amount'] = $appliedTax[$appliedTaxKey]['tax_amount'] + $line->quantity * round($singleProductTax, 2);
                    } else {
                        $singleProductTax = $line->unit_price * $tax_details->amount / 100;
                        $appliedTax[] = [
                            'tax_name' => $tax_details->name,
                            'tax_percent' => $tax_details->amount,
                            'tax_amount' => $line->quantity * round($singleProductTax, 2),
                        ];
                    }
                }
            }
            $output['appliedTax'] = $appliedTax;
            $output['modifier_tax'] = array_sum($modifierTaxes);
        ...
        $output['discount_amount'] = $discount_amount;
        ...
        $output['discount'] = ($discount != 0) ? $this->num_f($transaction->discount_amount, $show_currency, $business_details) : 0;
        ...
        if ($transaction_type == 'sell_return') {
            ...
            $output['final_total'] = round($transaction->final_total, 2);
        } else {
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            $query = Variation::join('products AS p', 'variations.product_id', '=', 'p.id')
                ...
                ->where('variations.id', $variation->id);

            $product1 = $query->select(
            ...
            $product1->price = $product1->default_sell_price;
            $variations = $product1->default_sell_price;
            ...
            if (!empty($variation->product->tax) && $variation->product->tax_type == 'inclusive') {
                ...
                if (!empty($tax)) {
                    $variation->default_sell_price += $variation->default_sell_price * $tax->amount / 100;

                    $variation->default_sell_price = number_format($variation->default_sell_price, 2);
                    $product1->price = $variation->default_sell_price;
                }
            }
            //            dd(! empty($tax_details) ? round($this->num_f($line->quantity * ($line->unit_price * $tax_details->amount / 100),
            //                false, $business_details), 2) : null);
            ...
            $line_array = [
                ...
                //                'unit_price'                 => ceil($line->unit_price),
                'tax' => $this->num_f($line->item_tax, false, $business_details),
                'single_product_tax' => !empty($tax_details) ? $line->unit_price * $tax_details->amount / 100 : null,
                'tax_id' => $line->tax_id,
                ...
                'tax_amount' => !empty($tax_details) ? round($this->num_f($line->quantity * ($product1->sell_price_inc_tax * $tax_details->amount / 100), false, $business_details), 2) : null,

                //Field for 3rd column
                'unit_price_inc_tax' => $this->num_f($line->unit_price_inc_tax, false, $business_details),
                'unit_price_inc_tax_uf' => round($this->num_f($line->unit_price_inc_tax, false, $business_details), 2),
                'sell_price_inc_tax' => $this->num_f($line->unit_price_inc_tax, false, $business_details),
                'unit_price_exc_tax' => $this->num_f($line->unit_price, false, $business_details),
                'price_exc_tax' => number_format($line->quantity * $line->unit_price, 2),
                'sub_total_final' => ($line->unit_price_inc_tax * $line->quantity),
                'unit_price_before_discount' => $this->num_f($line->unit_price_before_discount, false,
                    $business_details),

                'sell_price_fnal' => $line->unit_price_inc_tax,

                //Fields for 4th column
                'line_total' => $this->num_f($line->unit_price_inc_tax * $line->quantity, false, $business_details),
                //                'line_total'                 => ceil($line->unit_price_inc_tax) * ceil($line->quantity),
                'line_total_uf' => round($this->num_f($line->unit_price_inc_tax * $line->quantity, false, $business_details), 2),

            ];

            $temp = [];

            if (!empty($product->product_custom_field1) && in_array('product_custom_field1',
                $product_custom_fields_settings)) {
                $temp[] = $product->product_custom_field1;
            }
            if (!empty($product->product_custom_field2) && in_array('product_custom_field2',
                $product_custom_fields_settings)) {
                $temp[] = $product->product_custom_field2;
            }
            if (!empty($product->product_custom_field3) && in_array('product_custom_field3',
                $product_custom_fields_settings)) {
                $temp[] = $product->product_custom_field3;
            }
            if (!empty($product->product_custom_field4) && in_array('product_custom_field4',
                $product_custom_fields_settings)) {
            ...
            if (!empty($line->modifiers)) {
                foreach ($line->modifiers as $modifier_line) {
                    ...
                    $modifier_tax = null;
                    $taxRate = null;
                    if (!is_null($variation->tax_id) && $variation->tax_type == 'inclusive') {
                        $taxRate = TaxRate::find($variation->tax_id);
                        $modifier_tax = $this->num_f($modifier_line->quantity, false, $business_details) * ($variation->default_sell_price * $taxRate->amount / 100);
                    }
                    ...
                    $modifier_line_array = [
                        ...
                        'single_modifier_tax' => !is_null($taxRate) ? ($modifier_line->unit_price * $taxRate->amount / 100) : null,
                        'tax_id' => $variation->tax_id,
                        'tax_amount' => !is_null($taxRate) ? $taxRate->amount : 0,
                        'tax_type' => $variation->tax_type,
                        'modifier_tax' => !is_null($modifier_tax) ? $modifier_tax + ($this->num_f($modifier_line->quantity,
                            false, $business_details) * $this->num_f($modifier_line->unit_price, false,
                            $business_details)) : ($this->num_f($modifier_line->quantity,
                            false, $business_details) * $this->num_f($modifier_line->unit_price, false,
                            $business_details)),

                        //Field for 2nd column
                        //                        'quantity' => $this->num_f($modifier_line->quantity, false, $business_details),
                        'quantity' => round($this->num_f($modifier_line->quantity, false,
                            $business_details), 2),
                        'units' => !empty($unit->short_name) ? $unit->short_name : '',

                        //Field for 3rd column
                        'unit_price_inc_tax' => $modifier_line->unit_price_inc_tax,
                        //                        'unit_price_inc_tax' => ceil($modifier_line->unit_price_inc_tax),
                        ...
                        'sub_total_final' => $this->num_f($modifier_line->unit_price_inc_tax, false, $business_details),

                        //Fields for 4th column
                        'line_total' => $this->num_f($modifier_line->unit_price_inc_tax * $line->quantity,
                            false, $business_details),
                        //                        'line_total'         => ceil($modifier_line->unit_price_inc_tax) * ceil($line->quantity),
                    ];
                    $modifierSubTotal = $modifier_line_array['unit_price_inc_tax'] + $modifier_line_array['single_modifier_tax'];
                    $modifier_line_array['sub_total_final'] = $modifierSubTotal * $modifier_line_array['quantity'];
    ...
    protected function _receiptDetailsSellReturnLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            $line_array = [
                ...
                'unit_price' => round($this->num_f($line->unit_price, false, $business_details), 2),
                'tax' => $this->num_f($line->item_tax, false, $business_details),
                'tax_name' => !empty($tax_details) ? $tax_details->name : null,

                //Field for 3rd column
                'unit_price_inc_tax' => $this->num_f($line->unit_price_inc_tax, false, $business_details),
                'sell_price_inc_tax' => $this->num_f($line->sell_price_inc_tax, false, $business_details),
                'unit_price_exc_tax' => $this->num_f($line->unit_price, false, $business_details),

                //Fields for 4th column
                'line_total' => $this->num_f($line->unit_price_inc_tax * $line->quantity_returned, false,
                    $business_details),
            ];
    ...
    public function getPurchaseProducts($business_id, $transaction_id)
    {
        $products = Transaction::join('purchase_lines as pl', 'transactions.id', '=', 'pl.transaction_id')
            ...
            ->select('p.id as product_id', 'p.name as product_name', 'v.id as variation_id', 'v.name as variation_name',
    ...
    public function getSellTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        ...
        $output['final_total'] = $sell_details->sum('final_total');
    ...
    public function getSellsLast30Days($business_id, $group_by_location = false)
    {
        $query = Transaction::leftjoin('transactions as SR', function ($join) {
            ...
            ->where('transactions.type', 'sell')
            ->where('transactions.status', 'final')
            ->whereBetween(DB::raw('date(transactions.transaction_date)'),
    ...
    public function adjustMappingPurchaseSell($status_before, $transaction, $business, $deleted_line_ids = [])
    {
        ...
        if ($status_before == 'final' && $transaction->status == 'draft') {
            ...
            $sell_purchases = Transaction::join('transaction_sell_lines AS SL', 'transactions.id', '=',
                'SL.transaction_id')
    ...
    public function deleteSale($business_id, $transaction_id)
    {
        ...
        if (!empty($transaction)) {
            ...
            if ($transaction->status == 'draft') {
                ...
                Transaction::where('id', $transaction_id)->update($transactinsUpdate);
    ...
    public function getLedgerDetails($contact_id, $start, $end)
    {
        ...
        $payments = $this->__paymentQuery($contact_id, $start, $end)
            ->select('transaction_payments.*', 'bl.name as location_name', 't.type as transaction_type', 't.ref_no',
                't.invoice_no')
            ->get();
     ...
     * @return boolean/object
     */
    public function createOrUpdateSellLinesTransaction($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        $lines_formatted = [];
        $modifiers_array = [];
        $edit_ids = [0];
