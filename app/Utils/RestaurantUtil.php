...
class RestaurantUtil extends Util
{
    ...
    {
        $query = Transaction::with('sell_lines.product')->leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ->leftjoin(
        ...
        $locationIds = auth()->user()->permitted_locations();
      
        if (!empty($locationIds)){
            if ($locationIds == 'all'){
                $locationIds = null;
            }else{
                $query->whereIn('transactions.location_id',$locationIds);
            }
        }else{
            $query->whereIn('transactions.location_id',[]);
        }
        
        if ( !empty(auth()->user()->kitchen_station_id)) {
            $query->whereHas('sell_lines', function (Builder $q) use ($filter) {
                $q->whereHas('product', function (Builder $q) use ($filter) {
                    $q->whereHas('productKitchenStation', function (Builder $q) use ($filter) {
                        $q->where('kitchen_station_id', '=', auth()->user()->kitchen_station_id);
                    });
                });
            });
        }else{
            ...
            if ($location != 'all') {
                $query->whereHas('sell_lines', function (Builder $q) use ($filter) {
                    $q->whereHas('product', function (Builder $q) use ($filter) {
                        $q->whereHas('productKitchenStation', function (Builder $q) use ($filter) {
                            $q->where('kitchen_station_id', '=', null);
                        });
                    });
                });
            }
