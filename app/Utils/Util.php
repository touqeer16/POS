...
class Util
{
     ...
     * @throws \Exception
     */
    public function uploadCategoryFile($request, $file_name, $dir_name, $file_type = 'image')
    {
        //If app environment is demo return null
        if (config('app.env') == 'demo') {
            return null;
        ...
        if ($request->hasFile($file_name) && $request->file($file_name)->isValid()) {
            ...
            if ($request->$file_name->getSize() <= (int) config('constants.document_size_limit')) {
                $new_file_name = time() . '_' . $request->$file_name->getClientOriginalName();
                if ($request->$file_name->storeAs($dir_name, $new_file_name)) {
                    $uploaded_file_name = $new_file_name;
                }
    ...
    public function getServiceStaff($business_id, $location_id = null, $for_dropdown = false)
    {
        ...
        if (! empty($service_staff_roles)) {
            ...
            if ($for_dropdown) {
                $waiters = $waiters->select('id',
                    DB::raw('CONCAT(COALESCE(first_name, ""), " ", COALESCE(last_name, "")) as full_name'))->get()->pluck('full_name',
                    'id');
            } else {
                $waiters = $waiters->get();
            }
     ...
     * @return string
     */
    public function showCurrencySymbol($number, $add_symbol)
    {
        $formatted = '';
        if ($add_symbol) {
            $currency_symbol_placement = ! empty($business_details) ? $business_details->currency_symbol_placement : session('business.currency_symbol_placement');
            ...
            if ($currency_symbol_placement == 'after') {
                $formatted = ceil($number).' '.$symbol;
            } else {
                $formatted = $symbol.' '.ceil($number);
            }
        }
