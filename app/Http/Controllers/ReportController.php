...
class ReportController extends Controller
{
    ...
    public function getproductSellReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $category_id = $request->get('product_sell_category_id', null);
            ...
            return '<span data-is_quantity="true" class="display_currency sell_qty" data-currency_symbol=false data-orig-value="' . (float) $row->sell_qty . '" data-unit="' . $row->unit . '" >' . (float) $row->sell_qty . '</span> ' . $row->unit;
                ...
                ->editColumn('sell_qty', function ($row) {
                    return '<span data-is_quantity="true" class="display_currency sell_qty" data-currency_symbol=false data-orig-value="' . (float) $row->sell_qty . '" data-unit="" >' . (float) $row->sell_qty . '</span> ';
                })
                ->editColumn('subtotal', function ($row) {
                    return '<span class="display_currency row_subtotal" data-currency_symbol = true data-orig-value="' . $row->subtotal . '">' . $row->subtotal . '</span>';
                })
    ...
    public function getproductSellReportWithPurchase(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return '<span data-is_quantity="true" class="display_currency purchase_quantity" data-currency_symbol=false data-orig-value="' . (float) $row->purchase_quantity . '" data-unit="' . $row->unit . '" >' . (float) $row->purchase_quantity . '</span> ' . $row->unit;
                ...
                ->editColumn('purchase_quantity', function ($row) {
                    return '<span data-is_quantity="true" class="display_currency purchase_quantity" data-currency_symbol=false data-orig-value="' . (float) $row->purchase_quantity . '" data-unit="' . $row->unit . '" >' . (float) $row->purchase_quantity . '</span> ';
                })
                ->editColumn('ref_no', '
    ...
    public function getproductSellGroupedBranchByBranchReport(Request $request)
    {
        if (!auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()) {
            ...
            $start_date = $request->get('start_date');
            $end_date = $request->get('end_date');
            $start_date_column = date('d-m-Y', strtotime($start_date));
            $end_date_column = date('d-m-Y', strtotime($end_date));

            $transaction_date = '(' . $start_date_column . ') ' . ' To (' . $end_date_column . ')';
            if (!empty($start_date) && !empty($end_date)) {
                $query->where('t.transaction_date', '>=', $start_date)
                    ->where('t.transaction_date', '<=', $end_date);
            }
    ...
    public function getcategorySellGroupedBranchByBranchReport(Request $request)
    {
        if (!auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
    ...
    public function getproductSellGroupedReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->editColumn('business_location', function ($row) use (&$location_id) {
                    if (!empty($location_id)) {
                        return $row->business_location;
