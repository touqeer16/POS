...
class AdminSidebarMenu
{
    ...
    protected $stocks_module;
    protected $reports_module;
    protected $sell_payment_reports;
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            $this->stocks_module = (isset($package['package_details']['stocks_module']));
            $this->reports_module = (isset($package['package_details']['reports_module']));
            $this->sell_payment_reports = (isset($package['package_details']['sell_payment_reports']));

            /* echo "<pre>"; print_r($package['package_details']);exit; */
            ...
            if (auth()->user()->can('product.view') || auth()->user()->can('product.create') ||
                ...
                $menu->dropdown(
                    __('sale.products'),
                    function ($sub) use ($enabled_modules, $common_settings, $enable_brand) {
                        ...
                        if (auth()->user()->can('sell.create')) {
                            if (in_array('pos_sale', $enabled_modules)) {
                                $sub->url(
                                    ...
                                    __('sale.pos_sale'),
                                    ['icon' => 'fa fas fa-plus-circle hide', 'active' => request()->segment(1) == 'pos' && request()->segment(2) == 'create']
                                );
                            }
                        }
                        /*  if (auth()->user()->can('sell.create')) {
            ...
            /* Reprts allowed in packges */
            if ($this->reports_module) {
                /* Reports dropdown */
                    ...
                    $menu->dropdown(
                        ...
                        function ($sub) use ($enabled_modules) {
                            ...
                            if (auth()->user()->can('purchase_n_sell_report.view')) {
                                ...
                                /* Sell Payment Reports */

                                if ($this->sell_payment_reports) {
                                    $sub->url(
