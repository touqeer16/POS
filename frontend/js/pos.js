$(document).ready(function() {
    ...
    $('table#pos_table tbody').on('click', 'td.pos_remove_row', function() {
        $(this)
        .parents('tr')
        .remove();
        pos_total_row();
    ...
    // category product button value
    // let product_category_button = 'all';
    $('.product_category_button').on('click',function (){
    ...
    $('select#product_brand, select#select_location_id').on('change', function(e) {
        $('input#suggestion_page').val(1);
        var location_id = $('input#location_id').val();
    ...
    // delete all suspended sales
    $(document).on('click', '.delete-all-sale-button', function() {
        let value = $(this).val();
        $.ajax({
            method: 'POST',
            url: '/pos/suspended-sales',
            data: { value:value},
            success: function(result) {
                if (result.success) {
                    $('div.suspendedSalesDiv').hide();
                    $('button.delete-all-sale-button').hide();
                    $('p.noRecordFound').html('No records found');
                }
            },
        });
    });
    ...
    $(document).on('click', '#calculatorButton', function (){
        $('.input_quantity:last').val($('input#code').val());
        if($('.input_quantity:last').hasClass('input_quantity')){
            $('.input_quantity:last').change();
        }
        $('input#code').val('');
    });

    // calculator clear coding
    $(document).on('click','.calculatorClearButton',function (){
        let removeVal = $('.input_quantity:last').val();
        if($('table#pos_table').find('tr.product_row:last').length > 0) {
            if (removeVal.length == 1) {
                $('.input_quantity:last').val(0);
                $('.input_quantity:last').change();
            }
            if ($('.input_quantity:last').val() > 9) {
                $('.input_quantity:last').val(removeVal.slice(0, -1));
                $('.input_quantity:last').change();
            } else if (removeVal > 0) {
                $('.input_quantity:last').val(0);
            }
            if (removeVal == 0) {
                $('table#pos_table').find('tr.product_row:last').remove();
            }
        }
    });
...
$('div#product_list_body').on('scroll', function() {
    if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
        ...
        var category_id = $('.product_category_button').val();
