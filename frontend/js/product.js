...
$(document).ready(function () {
    ...
    $(document).on('change', 'input#single_dsp_inc_tax', function (e) {
        ...
        tax_rate = tax_rate == undefined ? 15 : tax_rate;
        ...
        var tax_applied = ((selling_price_inc_tax * tax_rate) / 100);

        __write_number($('input#single_dpp'), selling_price);
        __write_number($('input#single_dsp'), selling_price);
        __write_number($('input#applicable_tax'), tax_applied);
...
$(document).ready(function () {
    $(document).on('change', '.Location', function () {
        let locationIds = $(this).val();
        $.each(locationIds, function (el, val) {
            $('#kitchenStationsGroup-' + val).show();
        })
    });

    $('.Location').on("select2:unselect", function (e) {
        $('#kitchenStationsGroup-' + e.params.data.id).hide();
        $('#kitchenStationsGroup-' + e.params.data.id).find('input').prop('checked', false).val('');
        $('#kitchenStationsGroup-' + e.params.data.id).find('.iradio_square-blue').removeClass('checked');
    });
})
