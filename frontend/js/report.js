$(document).ready(function () {
    ...
    if ($('#product_sr_date_filter').length == 1) {
        $('#product_sr_date_filter').daterangepicker(
            ...
            function (start, end) {
                ...
                product_sell_grouped_branch_by_branch_report.ajax.reload();
                category_sell_grouped_branch_by_branch_report.ajax.reload();
    ...
    product_sell_report = $('table#product_sell_report_table').DataTable({
        ...
        ajax: {
            ...
            data: function (d) {
                ...
                d.location_id = $('select#location_id').val();
                d.product_sell_category_id = $('select#product_sell_category_id').val();
            },
        },
        columns: [
            { data: 'product_name', name: 'p.name' },
    ...
    product_sell_grouped_report = $('table#product_sell_grouped_report_table').DataTable({
        ...
        columns: [
            { data: 'product_name', name: 'p.name' },
            { data: 'sub_sku', name: 'v.sub_sku' },
            { data: 'business_location', name: 'bl.business_location' },
            { data: 'transaction_date', name: 't.transaction_date' },
    ...
    product_sell_grouped_branch_by_branch_report = $('table#product_sell_grouped_branch_by_branch_report_table').DataTable({
        processing: true,
        serverSide: true,
        aaSorting: [[1, 'desc']],
        ajax: {
            url: '/reports/product-sell-grouped-branch-by-branch-report',
            data: function (d) {
                var start = '';
                var end = '';
                var start_time = $('#product_sr_start_time').val();
        ...
        fnDrawCallback: function (oSettings) {
            $('#footer_grouped_branch_by_branch_subtotal').text(
                sum_table_col($('#product_sell_grouped_branch_by_branch_report_table'), 'row_subtotal')
            );
            $('#footer_total_grouped_branch_branch_sold').html(
                __sum_stock($('#product_sell_grouped_branch_by_branch_report_table'), 'sell_qty')
            );
            __currency_convert_recursively($('#product_sell_grouped_branch_by_branch_report_table'));
        },
    });


    category_sell_grouped_branch_by_branch_report = $('table#category_sell_grouped_branch_by_branch_report_table').DataTable({
        processing: true,
        serverSide: true,
        aaSorting: [[1, 'desc']],
        ajax: {
            url: '/reports/category-sell-grouped-branch-by-branch-report',
            data: function (d) {
                var start = '';
                var end = '';
                var start_time = $('#product_sr_start_time').val();
        ...
        columns: [
            ...
            { data: 'location_id', name: 'bl.location_id' },
            { data: 'business_location', name: 'bl.business_location' },
            { data: 'transaction_date', name: 't.transaction_date' },
            { data: 'total_qty_sold', name: 'total_qty_sold', searchable: false },
            { data: 'subtotal', name: 'subtotal', searchable: false },
        ...
        fnDrawCallback: function (oSettings) {
            $('#footer_category_grouped_branch_by_branch_subtotal').text(
                sum_table_col($('#category_sell_grouped_branch_by_branch_report_table'), 'row_subtotal')
            );
            $('#footer_category_total_grouped_branch_branch_sold').html(
                __sum_stock($('#category_sell_grouped_branch_by_branch_report_table'), 'sell_qty')
            );
            __currency_convert_recursively($('#category_sell_grouped_branch_by_branch_report_table'));
        },
    });

   

    $('#product_sell_report_form #variation_id, #product_sell_report_form #location_id, #product_sell_report_form #customer_id, #product_sell_report_form #product_sell_category_id').change(function () {
        product_sell_report.ajax.reload();
        product_sell_grouped_report.ajax.reload();
        product_sell_grouped_branch_by_branch_report.ajax.reload();
        category_sell_grouped_branch_by_branch_report.ajax.reload();
